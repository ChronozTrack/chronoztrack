...
async function handleRequest(action: UserAction, { locals, request }: { locals: App.Locals; request: Request }) {
  const parsedData = await parseRequest<SettingsOptions>(request)
  const [resource, values] = Object.entries(parsedData)[0] as [AppOptionsType, Record<string, FormDataEntryValue>[]]
  ...
  if (action === 'create' && !canCreate(resource, locals.user) || action === 'update' && !canUpdate(resource, locals.user)) {
    return fail(403, { error: `User don't have permission to ${String(action).toUpperCase()} ${optionInfo.title}` })
  }

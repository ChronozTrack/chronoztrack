...
export class TableOptionsController<T extends TableOptionsType> {
	...
	public async updateOptions(
		...
		| { rows: T['$inferSelect'][]; error?: never }
		| { rows?: never; error: ZodError<Partial<T['$inferSelect']>[]> }
	> {
		...
		return {
			rows: await db
				.update(this.#tblOptions)
				.set(sqlUpdates)
				.where(inArray(this.#tblOptions.id, ids))
				.returning()
		};
	}
...
export async function getAppOptions(userAccess: UserAccess): Promise<SettingsOptions> {
	...
	return await db
		.batch([batchQuery[0], ...batchQuery.slice(1)])
		.then((data) =>
			Object.assign(optionsValues, Object.fromEntries(userOptions.map((opt, i) => [opt, data[i]])))
		)
		.catch((err) => {
			console.error(err);
			return optionsValues;
